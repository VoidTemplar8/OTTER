...
int main() {
	...
	Texture2DData::sptr boxMap = Texture2DData::LoadFromFile("images/sample.png");
	...
	Texture2D::sptr box = Texture2D::Create();
	box->LoadData(boxMap);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for(int ix = 0; ix < 4; ix++) {
			...
			materials[ix].Box->Bind(2);
			shader->SetUniform("u_Shininess", materials[ix].Shininess);
			shader->SetUniform("u_Mix", materials[ix].Derp);
